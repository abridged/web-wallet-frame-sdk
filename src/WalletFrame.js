...
export class FrameProvider {
  ...
  async createAccount(pubKey) {
    return await this.sdk.createAccount(pubKey).then((x) => {
      console.log('x2 postcreateAccount', x.address, `x.deployed=${x.deployed}`);
...
const handleMsg = async (data, acct, refiFrame, sdk, _this) => {
  ...
  const shortMethodName = method.replace('eth_', '');
  switch (method) {
    case 'request_funds':
    // TODO
    // CUSTOM ABI
    ...
    case 'eth_gasPrice':
      ...
      if (result.toHexString) result = result.toHexString();
    ...
    case 'eth_estimateGas':
      result = await _this.provider.perform(shortMethodName, { transaction: params });
      break;

    case 'eth_getLogs':
      result = await _this.provider.perform(shortMethodName, { filter: param0 });
      break;

    case 'eth_getEtherPrice':
      result = await _this.provider.perform(shortMethodName);
      break;

    case 'eth_getTransactionCount':
      result = await _this.provider.perform(shortMethodName, { address: param0 });
      break;
